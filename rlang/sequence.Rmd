...
sequence_plot <- function(tb, config){
  annotation = str_glue("City size={config$config_city_size} blocks<br>
                        ...
                        Trip inhomogeneity={config$config_inhomogeneity}")
...
tb <- sequences %>% 
  ...
  select(c(config_base_demand, config_inhomogeneity, mean_vehicle_count, measure, value)) %>%
  mutate(vehicles_per_trip = mean_vehicle_count / config_base_demand) %>%
  filter(config_inhomogeneity > 0.0)
