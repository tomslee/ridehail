...
def main():
    ...
    p2_fraction_increased = (
        results_3["vehicle_p2_fraction"] > results_0["vehicle_p2_fraction"]
    )
    wait_time_increased = results_3["trip_wait_time"] > results_0["trip_wait_time"]
    p3_fraction_unchanged = (
        abs(results_3["vehicle_p3_fraction"] - results_0["vehicle_p3_fraction"]) < 0.05
    )
    ...
    print("\nActual behavior:")
    print(
        f"  - Vehicle P2 fraction increased: {'✓ YES' if p2_fraction_increased else '✗ NO'}"
    )
    ...
    p3_status = (
        "✓ YES" if p3_fraction_unchanged else f"✗ NO (changed by {p3_change:.4f})"
    )
